...
class ExecuteBoundaryTest {
  ...
  @Test
  fun `raise in execute routes to onDomainError`(): Unit = runBlocking {
  ...
  @Test
  fun `orDie in execute routes to defect`(): Unit = runBlocking {
  ...
  @Test
  fun `unexpected Throwable in execute routes to defect`(): Unit = runBlocking {
    val capturedDefects = mutableListOf<Throwable>()
    val anchor = createAnchor(
      defect = { capturedDefects.add(it) },
    )
  ...
  @Test
  fun `CancellationException in execute is never swallowed`(): Unit = runBlocking {
    val capturedErrors = mutableListOf<TestError>()
    val capturedDefects = mutableListOf<Throwable>()
    val anchor = createAnchor(
      onDomainError = { capturedErrors.add(it) },
  ...
  @Test
  fun `missing onDomainError causes rethrow`(): Unit = runBlocking {
  ...
  @Test
  fun `missing defect handler causes rethrow`(): Unit = runBlocking {
  ...
  @Test
  fun `raise in init routes to onDomainError`(): Unit = runBlocking {
    val capturedErrors = mutableListOf<TestError>()
    val anchor = createAnchor(
  ...
  @Test
  fun `orDie in init routes to defect`(): Unit = runBlocking {
    val capturedDefects = mutableListOf<Throwable>()
    val anchor = createAnchor(
  ...
  @Test
  fun `unexpected Throwable in init routes to defect`(): Unit = runBlocking {
    val capturedDefects = mutableListOf<Throwable>()
    val anchor = createAnchor(
  ...
  @Test
  fun `RaisedException in subscription flow routes to onDomainError via catch`(): Unit = runBlocking {
    val capturedErrors = mutableListOf<TestError>()
    val anchor = createAnchor(
      onDomainError = { capturedErrors.add(it) },
    )
  ...
  @Test
  fun `DomainDefectException in subscription flow routes to defect via catch`(): Unit = runBlocking {
    val capturedDefects = mutableListOf<Throwable>()
    val anchor = createAnchor(
      defect = { capturedDefects.add(it) },
    )
  ...
  @Test
  fun `RaisedException never reaches defect handler`(): Unit = runBlocking {
    val capturedErrors = mutableListOf<TestError>()
    val capturedDefects = mutableListOf<Throwable>()
    val anchor = createAnchor(
      onDomainError = { capturedErrors.add(it) },
  ...
  @Test
  fun `DomainDefectException never reaches onDomainError`(): Unit = runBlocking {
    val capturedErrors = mutableListOf<TestError>()
    val capturedDefects = mutableListOf<Throwable>()
    val anchor = createAnchor(
      onDomainError = { capturedErrors.add(it) },
  ...
  @Test
  fun `handler can modify state during error recovery`(): Unit = runBlocking {
    val anchor = createAnchor(
      onDomainError = { error ->
        reduce { copy(value = -1) }
      },
